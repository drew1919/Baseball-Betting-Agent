...
Winner projections use the regression only after its training procedure beats the stored-pick baseline on chronologically later games. Approval must pass the full forward window, the latest five eligible slates, and a qualified-tier gate requiring at least 60% accuracy among at least 15 recent predictions rated 55% or higher. A recent accuracy drop automatically pauses production regression use and falls back to a weighted-statistical probability plus a bounded market sanity check: 15% for genuine two-sided odds, 9% for an estimated one-sided RotoWire market, and 0% when odds are unavailable. Probabilities are shrunk toward 50% to reduce overconfidence.

Regression training accepts only versioned, non-legacy snapshots with modern probability semantics, finite model features, evidence quality of at least 75%, confirmed/sufficiently matched lineups, full Statcast coverage for both starters, and both bullpen feeds. It requires at least 200 clean completed games before fitting, then removes constant predictors and predictors that are at least 92% correlated with a higher-priority feature inside each training window. Legacy rows remain in history for accountability and descriptive benchmarks, but cannot train, validate, or reactivate production. This prevents rows captured before bullpen, defense, evidence-quality, and immutable prediction fields existed from distorting current coefficients.
...
Each immutable snapshot stores the exact market weight used for its prediction. Historical replay reads that stored value, preventing an estimated 9% RotoWire blend from being incorrectly reevaluated later as a full 15% two-sided market blend.
...
Run `npm run validate:model -- data/app.db` to evaluate the primary and selective regressions with rolling daily holdouts, recent-slate approval metrics, dual-model agreement, and the 55%+ qualified tier separately from forced low-confidence picks. Run `npm run analyze:history -- data/app.db` for recent performance, market agreement, side bias, and feature-variance diagnostics. The regression report also exposes feature-removal decisions and the observed variation contributed by offense, pitching, trend, win probability, defense, and park so configured weights cannot silently behave like different practical weights. Run `npm run analyze:market-blend -- data/app.db` to compare bounded market weights and category directionality without changing production weights. Run `npm run calibrate:rotowire -- data/app.db` to refit/check the one-sided-line hold correction against stored two-sided markets. These are forward-looking checks; in-sample accuracy is not used as evidence for promotion.
